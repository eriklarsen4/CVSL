...
for (i in 1:nrow(Player_Batting_ALL2)) {
  if (i %% 17 == 0) {
...
Hitter_splits3 = purrr::reduce(mget(ls()[which(grepl(ls(), pattern = "batting_splits") == T)]), full_join)
rm(list = ls()[which(grepl(ls(), pattern = "batting_splits") == T)])
...
Pitcher_splits3 = purrr::reduce(mget(ls()[which(grepl(ls(), pattern = "pitching_splits") == T)]), full_join)
...
LgAvg_Pitcher_Splits = Pitcher_splits %>%
  ...
  dplyr::mutate(across(c("G", "PA", "AB", "R", "H", "2B", "3B", "HR", "SB", "CS", "BB", "SO", "TB", "GDP", "HBP", "SH", "SF", "IBB", "ROE", "tOPS+", "sOPS+"), round, 0)) %>%
  full_join(Pitcher_splits) %>%
  dplyr::arrange(OPS)
...
LgAvg_Hitter_splits = Hitter_splits %>%
  ...
  dplyr::mutate(bbref_id = "LgAvg", .before = 1) %>%
  dplyr::mutate(across(c("BA", "OBP", "SLG", "OPS", "BAbip", "tOPS+", "sOPS+"), round, 3)) %>%
  dplyr::mutate(across(c("G", "PA", "AB", "R", "H", "2B", "3B", "HR", "RBI", "SB", "CS", "BB", "SO", "TB", "GDP", "HBP", "SH", "SF", "IBB", "ROE", "tOPS+", "sOPS+"), round, 0)) %>%
  full_join(Hitter_splits) %>%
  dplyr::arrange(OPS)
...
Hitter_splits2 = Hitter_splits %>%
  ...
  dplyr::relocate(Name, .before = "bbref_id") %>%
  dplyr::relocate(playerid, .after = "Name") %>%
...
Player_Batting_ALL2 = Player_Batting_ALL2 %>%
  ...
  dplyr::mutate(Name = Name.x,
                bWAR = WAR) %>%
  dplyr::relocate(Name, .before = lastinit) %>%
  dplyr::relocate(bWAR, .before = RAR) %>%
  dplyr::select(-contains("key"), -contains("name_"), -contains("mlb"), -Name.x, -Name.y, -WAR)
...
rm(list = ls()[which(grepl(ls(), pattern = "CVSL Draft") == T)])
```
...
CVSL_2019_DRAFT = dummy %>%
  dplyr::mutate(Salary = cbind(c(CVSL_2019_DRAFT[,2],
                                 CVSL_2019_DRAFT[,4],
                                 CVSL_2019_DRAFT[,6],
                                 CVSL_2019_DRAFT[,8],
                                 CVSL_2019_DRAFT[,10],
  ...
  dplyr::mutate(CVSLTeam = case_when(Name %in% CVSL_2019_DRAFT$`Golden Bears` &
                                       Name != "" ~ "Golden Bears",
                                     Name %in% CVSL_2019_DRAFT$Phillies &
                                       Name != "" ~ "Phillies",
                                     Name %in% CVSL_2019_DRAFT$Nuggets &
                                       Name != "" ~ "Nuggets",
                                     Name %in% CVSL_2019_DRAFT$Cattlemen &
                                       Name != "" ~ "Cattlemen",
                                     Name %in% CVSL_2019_DRAFT$Bison &
                                       Name != "" ~ "Bison",
                                     Name %in% CVSL_2019_DRAFT$Arsenal &
                                       Name != "" ~ "Arsenal",
                                     Name %in% CVSL_2019_DRAFT$Tamales &
                                       Name != "" ~ "Tamales",
                                     Name %in% CVSL_2019_DRAFT$Matadors &
                                       Name != "" ~ "Matadors",
                                     Name %in% CVSL_2019_DRAFT$Dodgers &
                                       Name != "" ~ "Dodgers",
                                     Name %in% CVSL_2019_DRAFT$Ducks &
                                       Name != "" ~ "Ducks"))
rm(dummy)
...
CVSL_2020_DRAFT = dummy %>%
  dplyr::mutate(Salary = cbind(c(CVSL_2020_DRAFT[,2],
                                 CVSL_2020_DRAFT[,4],
                                 CVSL_2020_DRAFT[,6],
                                 CVSL_2020_DRAFT[,8],
                                 CVSL_2020_DRAFT[,10],
                                 ...
                                 CVSL_2020_DRAFT[,20]))) %>%
  dplyr::filter(!is.na(Salary)) %>%
  dplyr::mutate(CVSLTeam = case_when(Name %in% CVSL_2020_DRAFT$Phillies &
                                       Name != "" ~ "Phillies",
                                     Name %in% CVSL_2020_DRAFT$Ducks &
                                       Name != "" ~ "Ducks",
                                     Name %in% CVSL_2020_DRAFT$Bison &
                                       Name != "" ~ "Bison",
                                     Name %in% CVSL_2020_DRAFT$Dodgers &
                                       Name != "" ~ "Dodgers",
                                     Name %in% CVSL_2020_DRAFT$Bears &
                                       Name != "" ~ "Bears",
                                     Name %in% CVSL_2020_DRAFT$Matadors &
                                       Name != "" ~ "Matadors",
                                     Name %in% CVSL_2020_DRAFT$Cattlemen &
                                       Name != "" ~ "Cattlemen",
                                     Name %in% CVSL_2020_DRAFT$Nuggets &
                                       Name != "" ~ "Nuggets",
                                     Name %in% CVSL_2020_DRAFT$Copperheads &
                                       Name != "" ~ "Copperheads",
                                     Name %in% CVSL_2020_DRAFT$Tamales &
                                       Name != "" ~ "Tamales"))
rm(dummy)
...
CVSL_2021_DRAFT = dummy %>%
  dplyr::mutate(Salary = cbind(c(CVSL_2021_DRAFT[,2],
                                 CVSL_2021_DRAFT[,4],
                                 CVSL_2021_DRAFT[,6],
                                 CVSL_2021_DRAFT[,8],
                                 CVSL_2021_DRAFT[,10],
                                 ...
                                 CVSL_2021_DRAFT[,20]))) %>%
  dplyr::filter(!is.na(Salary)) %>%
  dplyr::mutate(CVSLTeam = case_when(Name %in% CVSL_2021_DRAFT$Phillies &
                                       Name != "" ~ "Phillies",
                                     Name %in% CVSL_2021_DRAFT$Ducks &
                                       Name != "" ~ "Ducks",
                                     Name %in% CVSL_2021_DRAFT$Bison &
                                       Name != "" ~ "Bison",
                                     Name %in% CVSL_2021_DRAFT$Dodgers &
                                       Name != "" ~ "Dodgers",
                                     Name %in% CVSL_2021_DRAFT$Renegades &
                                       Name != "" ~ "Renegades",
                                     Name %in% CVSL_2021_DRAFT$Matadors &
                                       Name != "" ~ "Matadors",
                                     Name %in% CVSL_2021_DRAFT$Cattlemen &
                                       Name != "" ~ "Cattlemen",
                                     Name %in% CVSL_2021_DRAFT$Nuggets &
                                       Name != "" ~ "Nuggets",
                                     Name %in% CVSL_2021_DRAFT$Copperheads &
                                       Name != "" ~ "Copperheads",
                                     Name %in% CVSL_2021_DRAFT$Tamales &
                                       Name != "" ~ "Tamales"))
rm(dummy)

# Clean the 2022 df ----
  ## Remove the first 4 rows and the extra columns
CVSL_2022_DRAFT = CVSL_2022_DRAFT[ -c(1:4), -c(seq(7, ncol(CVSL_2022_DRAFT), 8)) ]

dummy = CVSL_2022_DRAFT %>%
  dplyr::select(-contains("X"), -contains("Players")) %>%
  dplyr::slice(-c(1:4)) %>%
  dplyr::mutate(across(contains("Cap"), ~ gsub(., pattern = "\\$", replacement = "")))

CVSL_2022_DRAFT = pivot_longer(dummy, cols = everything()) %>%
...
```{r Import Blind Bid history, include = F, message = F, warning = F, echo = F}
...
Hitter_splits %>%
  ...
  dplyr::select(Name, bbref_id, Split, Year,
                G, PA, AB, R, H, `2B`, `3B`, HR, RBI,
                SB, CS, BB, SO, BA, OBP, SLG, OPS, TB,
                GDP, HBP, SH, SF, IBB, ROE, BAbip, `tOPS+`, `sOPS+`) %>%
  dplyr::filter(Year == 2022, Split == "vs LHP") %>%
  dplyr::arrange(OPS)
```