...
```{r load splits environment, include = T, message = F, warning = F, echo = F}
...
DRAFTS = DRAFTS %>%
  dplyr::mutate(labs = case_when(Salary >= 35 ~ Name,
                                 TRUE ~ ""))
...
DRAFTS %>%
  ...
  labs(title = "CVSL Auctions, Hitters (2019-2022)", x = "'RBat'", y = "Salary ($)")+
  guides(alpha = "none")+
  guides(size = "none")+
  theme_bw()+
