...
Scrape hitting data from `Baseball Reference` for relevant years

```{r Scrape BR hitting ata, eval = F, include = T, message = F, warning = F, echo = T}
...
```{r Extract hitters names and add playerids, include = T, message = F, warning = F, echo = T}
...
```{r Correct hittername spellings, eval = F, include = T, message = F, warning = F, echo = F}
...
Scrape pitching data from `Baseball Reference` for relevant years
...
```{r Scrape Individual pitchers pages, eval = F, include = F, message = F, warning = F, echo = T}
...
```{r Integrate the pitchers changers data, eval = T, include = T, message = F, warning = F, echo = T}
  ## join all hitters and pitchers who changed teams
# Team_Transactions_ALL = Team_Changers_hitters %>%
  # full_join(Team_Changers_pitchers)
...
for (i in 3815:nrow(Player_Batting_ALL2)) {
  if (i %% 17 == 0) {
  ...
  ## extract the splits for each of those omitted datasets
BR_PLAYER_SPLITS_IMPORT_fn(Year = 23, Player_data_type = "batting", playerid = "altuvjo01")
BR_PLAYER_SPLITS_IMPORT_fn(Year = 23, Player_data_type = "batting", playerid = "alvarfr01")
BR_PLAYER_SPLITS_IMPORT_fn(Year = 23, Player_data_type = "batting", playerid = "alvaryo01")
BR_PLAYER_SPLITS_IMPORT_fn(Year = 23, Player_data_type = "batting", playerid = "andujmi01")
BR_PLAYER_SPLITS_IMPORT_fn(Year = 23, Player_data_type = "batting", playerid = "caratvi01")
BR_PLAYER_SPLITS_IMPORT_fn(Year = 23, Player_data_type = "batting", playerid = "diazel01")
...
rm(list = ls()[which(grepl(ls(), pattern = "batting_splits_") == T)])
  ...
  ## Fix the aggregated df that has those Latin-1 encoding errors; Altuve, Yordan, Andujar, Francisco Alvarez, Victor Caratini, Elias Diaz
Player_Batting_ALL2$bbref_id[3693] = batting_splits_3693$bbref_id
Player_Batting_ALL2$lastinit[3693] = str_extract(batting_splits_3693$bbref_id, pattern = "[a-z]{1}")

Player_Batting_ALL2$bbref_id[3695] = batting_splits_3695$bbref_id
Player_Batting_ALL2$lastinit[3695] = str_extract(batting_splits_3695$bbref_id, pattern = "[a-z]{1}")

Player_Batting_ALL2$bbref_id[3696] = batting_splits_3696$bbref_id
Player_Batting_ALL2$lastinit[3696] = str_extract(batting_splits_3696$bbref_id, pattern = "[a-z]{1}")

Player_Batting_ALL2$bbref_id[3701] = batting_splits_3701$bbref_id
Player_Batting_ALL2$lastinit[3701] = str_extract(batting_splits_3701$bbref_id, pattern = "[a-z]{1}")

Player_Batting_ALL2$bbref_id[3769] = batting_splits_3769$bbref_id
Player_Batting_ALL2$lastinit[3769] = str_extract(batting_splits_3769$bbref_id, pattern = "[a-z]{1}")

Player_Batting_ALL2$bbref_id[3814] = batting_splits_3814$bbref_id
Player_Batting_ALL2$lastinit[3814] = str_extract(batting_splits_3814$bbref_id, pattern = "[a-z]{1}")

## Add searchable names to the splits df
Hitter_splits = Hitter_splits %>%
  dplyr::select(-GS, -I) %>%
  dplyr::rename(key_bbref = bbref_id) %>%
  inner_join(CHAD_LU, by = "key_bbref") %>%
  dplyr::rename(bbref_id = key_bbref) %>%
  dplyr::select(-contains("name_"), -contains("key_"), -contains("mlb")) %>%
  dplyr::mutate(Nameo = Name, .before = 1) %>%
  dplyr::select(-Name) %>%
  dplyr::rename(Name = Nameo)
```

## Scrape Splits Data

Show the scrape code (data imported)
...
```{r group splits, eval = T, include = T, message = F, warning = F, echo = T}

## Get the handedness of the pitcher
Pitcher_splits = Pitcher_splits %>%
  ...
  dplyr::select(c(1:30,34))

## Group by platoon year to derive lg avg splits
LgAvg_Pitcher_Splits = Pitcher_splits %>%
  dplyr::mutate(across(c(3:29), as.numeric)) %>%
  dplyr::group_by(Throws, Split, Year) %>%
  dplyr::summarize(across(where(is.numeric), mean, na.rm = T)) %>%
  dplyr::ungroup() %>%
...
Hitter_splits = Hitter_splits %>%
  dplyr::mutate(key_bbref = bbref_id) %>%
  left_join(Player_Batting_ALL2, by = c("bbref_id", "Year")) %>%
  dplyr::relocate(Bats, .after = 2) %>%
  dplyr::relocate(Age, .after = 3) %>%
  dplyr::relocate(Tm, .after = 4) %>%
  dplyr::relocate(Traded, .after = 5) %>%
  dplyr::relocate(Positions, .after = 6) %>%
  dplyr::relocate(Name, .before = 1) %>%
  dplyr::relocate(Year, .before = 6) %>%
  dplyr::rename_at(vars(contains(".x")), ~ str_remove(., ".x")) %>%
  dplyr::select(c(1:37), -GS) %>%
  dplyr::mutate(lastinit = str_extract(bbref_id, pattern = "[a-z]{1}"), .after = 3)

## Group by platoon year to derive lg avg splits
LgAvg_Hitter_splits = Hitter_splits %>%
  dplyr::group_by(Bats, Split, Year) %>%
  dplyr::summarize(across(where(is.numeric), mean, na.rm = T)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(bbref_id = "LgAvg", .before = 1) %>%
...
```{r Add FG IDs, eval = T, include = F, message = F, warning = F, echo = F}
## Add FG IDs to the hitter splits
Hitter_splits2 = Hitter_splits %>%
  dplyr::mutate(key_bbref = bbref_id) %>%
  left_join(CHAD_LU, by = c("key_bbref")) %>%
  dplyr::mutate(playerid = key_fangraphs) %>%
  dplyr::rename(Name = Name.x) %>%
  dplyr::relocate(playerid, .after = "bbref_id") %>%
  dplyr::relocate(lastinit, .after = "bbref_id") %>%
  dplyr::select(-contains("key_"), -contains("name_"), -contains("mlb"), -contains(".y"))

## Add FG IDs to the pitcher splits
Pitcher_splits2 = Pitcher_splits %>%
  dplyr::mutate(key_bbref = bbref_id) %>%
  left_join(CHAD_LU, by = "key_bbref") %>%
  dplyr::mutate(playerid = key_fangraphs) %>%
  dplyr::rename(Name = Name.x) %>%
  dplyr::relocate(playerid, .after = "bbref_id") %>%
  dplyr::mutate(lastinit = str_extract(bbref_id, pattern = "[a-z]{1}"), .after = "bbref_id") %>%
  dplyr::select(-contains("key_"), -contains("name_"), -contains("mlb"))
...
```{r clean the GE, include = T, message = F, warning = F, echo = F}
rm(list = ls()[which(grepl(ls(), pattern = "Changing|df|Hitter_splits$|LgAvg|Pitcher_splits$|Player_(.+?)_ALL$|Team_") == T)])

Hitter_splits = Hitter_splits2
rm(Hitter_splits2)

Pitcher_splits = Pitcher_splits2
rm(Pitcher_splits2)
...
DRAFTS = DRAFTS %>%
  ...
  dplyr::mutate(bbref_id = case_when(grepl(Name, pattern = "Gorkys") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Anibal Sanchez") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Kiner") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Jose Alverado") ~ CHAD_LU %>%
                                       dplyr::filter(grepl(Name, pattern = "Jos") &
                                     ...
                                     grepl(Name, pattern = "Ponce") ~ CHAD_LU %>%
                                       dplyr::filter(grepl(Name, pattern = "Daneil P")) %>%
                                       dplyr::select(key_bbref) %>%
                                       as.character(),
                                     ...
                                     grepl(Name, pattern = "Elieser Hernandez") ~ CHAD_LU %>%
                                       dplyr::filter(grepl(Name, pattern = "Elieser")) %>%
                                       dplyr::select(key_bbref) %>%
                                       as.character(),
                                     ...
                                     grepl(Name, pattern = "Adam Civale") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Oscar Mercado") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "John Merryweather") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Adam Ott") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Christian V") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Teoscar") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Noe Ramirez") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Daniel") &
                                       grepl(Name, pattern = "Leon") ~ CHAD_LU %>%
                                       dplyr::filter(grepl(Name, pattern = "ponce de leon")) %>%
                                       dplyr::select(key_bbref) %>%
                                       as.character(),
                                     
                                     grepl(Name, pattern = "Andres") &
                                     ...
                                     grepl(Name, pattern = "Nick Castellanos") ~ CHAD_LU %>%
                                       dplyr::filter(grepl(Name, pattern  = "Ni") &
                                                       grepl(Name, pattern = "Castellanos")) %>%
                                       dplyr::select(key_bbref) %>%
                                       as.character(),
                                     ...
                                     grepl(Name, pattern = "Quijada") ~ CHAD_LU %>%
                                     ...
                                     grepl(Name, pattern = "Adrian Morejon") ~ CHAD_LU %>%
...
DRAFTS = DRAFTS %>%
  dplyr::mutate(key_bbref = bbref_id) %>%
  anti_join(CHAD_LU, by = "key_bbref") %>%
...
## Plot Draft Results
```{r Join Draft history with stats, include = T, message = F, warning = F, echo = F}
...
DRAFTS %>%
  ...
  geom_point(aes(x = Rbat,
                 ...
                 color = CVSLTeam)) +
  facet_wrap(~Year)+
  labs(title = "CVSL Auction Draft History, 2019-2021\nHitters", x = "RBat, aka wRC+", y = "Salary ($)")+
  guides(alpha = "none")+
  guides(size = "none")+
  theme_bw()+
...
```{r CVSL Eligible Teams, eval = F, include = T, message = F, warning = F, echo = F}
...
+ `C`s need **>= 100 ABs** to be eligible to catch in games
+ all other hitters need **>= 175 ABs** to be eligible to play in games

Filter pitchers by:

+ `SP`s need **>= 8 starts (GS)** *AND* **>= 30 IP** to be eligible to start in games
+ `RP`s need **>= 30 IP** to be eligible to pitch in games
