...
This snippet was used in [my article](https://www.theleftycatcher.com/post/diving-deep-into-tanking) on [TheLeftyCatcher](https://www.theleftycatcher.com), where I compared and contrasted how `Jeff Luhnow` and `Theo Epstein` tanked to rebuild the `Chicago Cubs` and `Houston Astros` into World Series winners.
...
It required some processing to compile and analyze with or without graphs

The functions used are sourced from [this](https://github.com/eriklarsen4/Baseball/blob/main/Tanking/Tanking%20Functions.R) file

I created a [separate markdown file for the graphs](https://github.com/eriklarsen4/Baseball/blob/main/Tanking/Tanking-Graphs.md)
...
```{r Import Environment, eval = T, include = T, message = F, warning = F, echo = F}
load("~/GitHub/Baseball/Tanking/WranglingEnvironment.RData")
```

# Baseball Reference Standings Data {.tabset .tabset-pills .tabset-fade}

Import `Baseball Reference`'s `Standings` data over relevant years (`2011`-`2021`)

## Scrape and Clean Baseball Reference Standings Data

```{r Scrape standings data from Baseball Reference, eval = F, include = T, message = F, warning = F, echo = T}
...
```{r Import Player Value Data from BBRef, eval = F, include = T, warning = F, message = F, echo = T}
...
+ scraped data in these tables do not contain `Baseball Reference` `playerid`s

  + add `playerid`s from the `CHADWICK DB` in order to scrape individual player pages
    to account for players that changed teams within a given season

```{r Join BBRef data and correct columns, eval = F, include = T, warning = F, message = F, echo = T}
...
```{r Correct hitter name spellings and ids, eval = F, include = T, warning = F, message = F, echo = F}
...
```{r Correct pitcher name spellings and ids 2, eval = F, include = T, message = F, warning = F, echo = F}
...
```{r Summarize Player Career data, eval = F, include = T, warning = F, echo = T, message = F}
...
```{r Scrape team bWAR and salary/payroll data from Baseball Reference, eval = F, include = T, warning = F, echo = T, message = F}
...
```{r Identify players who changed teams, eval = F, include = T, warning = F, echo = T, message = F}
...
```{r Combine the changers data, eval = F, include = F, message = F, warning = F, echo = T}
...
```{r Add Team Abbrevs to Team Hitting Data, eval = T, include = T, message = F, warning = F, echo = F}
...
```{r Add Team Abbrevs to Team Pitching Data, eval = T, include = T, message = F, warning = F, echo = F}
TeamPitALL = TeamPitALL %>%
  dplyr::mutate(Team = Tm, .before = 1) %>%
  dplyr::mutate(Type = "Pit") %>%
  dplyr::mutate(Tm = case_when(grepl(Team, pattern = "Ariz") ~ "ARI",
...
```{r Join Team Transaction Data to Team Value Data, eval = T, include = T, message = F, warning = F, echo = T}
...
```{r Scrape and clean Free-Agent data from ESPN, include = T, message = F, warning = F, echo = T}
...
```{r Scrape and clean Intl Signing Data from Spotrac, include = T, message = F, warning = F, echo = T}
...
```{r Save data, include = T, message = F, warning = F, echo = F}
rm(
  list = ls()[
...
```{r Scrape and Clean Amateur draft data, eval = T, include = T, message = F, warning = F, echo = T}
  ## set the selector
...
# Data Summaries {.tabset .tabset-pills .tabset-fade}
...
```{r Show the Tank Draft table, include = T, message = F, warning = F, echo = F}
DT::datatable(TANK_DF)
```
...
```{r REBUILDERS_df and plotly creation, include = T, message = F, warning = F, echo = T, render = T}
