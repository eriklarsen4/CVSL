...
```{r Add player acquisition data to the Baseball Reference player value data pt.1, include = T, warning = F, echo = T, message = F}


Player_Value_Batting_ALL = Player_Value_Batting_ALL %>%
  dplyr::mutate(Positions = batPositions,
                Year = X) %>%
  dplyr::select(-X, -Pos.Summary, -idx)

Team_Changers_hitters = Player_Value_Batting_ALL %>%
  dplyr::filter(grepl(Tm, pattern = "TM")) %>%
  dplyr::select(Name, Year, Tm, Salary, Acquired) %>%
  dplyr::distinct()
...
Player_Value_Batting_ALL %>%
  dplyr::filter(grepl(Tm, pattern = "TM")) %>%
  dplyr::mutate(last_abbrev = str_to_lower(str_extract(playerid, pattern = "[a-z]{1}"))) %>%
  dplyr::select(Name, last_abbrev, Year, playerid) %>%
...
HITTING_LEADERS_BR$Positions = gsub(x = HITTING_LEADERS_BR$Positions, pattern = "\\*|\\/|D|H", replacement = "")
  ## Loop through and replace position code with abbrevs, splitting different positions by spaces
for (i in 1:nrow(HITTING_LEADERS_BR)) {
  listy[i] = str_extract_all(strsplit(HITTING_LEADERS_BR$Positions[i], split = "\\D"), pattern = "[:digit:]")
  for (j in 1:length(listy[[i]])){
    if (listy[[i]][j] == 2) {
      listy[[i]][j] = "C"
    }
  ...
  HITTING_LEADERS_BR$Positions[i] = paste(unlist(listy[[i]]), collapse = " ")
}
  ## Replace guys who only DH'd with "DH"
HITTING_LEADERS_BR$Positions[ which(HITTING_LEADERS_BR$Positions == "0") ] = "DH"
```
