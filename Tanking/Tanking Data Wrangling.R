...
BBA_DRAFT_ALL$`Drafted By`[ grep(BBA_DRAFT_ALL$`Drafted By`, pattern = "Senators") ] = "Oakland Athletics"
  ...
  ## Remove duplicates
#BBA_DRAFT_ALL = BBA_DRAFT_ALL[ which(duplicated(BBA_DRAFT_ALL$`Player Name`,  ) == FALSE), ] 
...
#Draft_df_Analysis(df = BBA2015, Team = "New York Yankees")
print(BBA_DRAFT_ALL[ which(BBA_DRAFT_ALL$Tm == "Houston Astros" & BBA_DRAFT_ALL$Rd <= 5 ), ], n = 36)
print(BBA_DRAFT_ALL[ which(BBA_DRAFT_ALL$Tm == "Chicago Cubs" & BBA_DRAFT_ALL$Rd <= 5), ], n = 52)

##### Standings Data from Baseball Reference #####
...
## Create functions to clean the hitter dfs
...
## Loop through and find position players in the player value pitching df
## Initialize vector
temp = vector()
...
## Put the cleaned Hitter Value dfs into one df for easier calculation
...
##### Team WAR and Salary Data from Baseball Reference #####
...
TeamPitALL$Salary = as.numeric(TeamPitALL$Salary)
  ...
  ## Add Player Acquisition data to the value data
  ## Iniitialize vector
df = vector()
...
##### Intl Signing Data from Spotrac ####
...
ASTROS_CUBS_DRAFT_SUB = BBA_DRAFT_ALL_no_dup[ which(BBA_DRAFT_ALL_no_dup$Tm == "Houston Astros" | BBA_DRAFT_ALL_no_dup$Tm == "Chicago Cubs") , ]
LEAGUE_DRAFT_SUB = BBA_DRAFT_ALL_no_dup[ which(BBA_DRAFT_ALL_no_dup$Tm != "Houston Astros" & BBA_DRAFT_ALL_no_dup$Tm != "Chicago Cubs"), ]

  ## Remove duplicates
#ASTROS_CUBS_DRAFT_SUB = ASTROS_CUBS_DRAFT_SUB[-which(duplicated(ASTROS_CUBS_DRAFT_SUB$Name) == TRUE),]
#LEAGUE_DRAFT_SUB = LEAGUE_DRAFT_SUB[-which(duplicated(LEAGUE_DRAFT_SUB$Name) == TRUE),]
...
CUBS_TANK_DF = as.data.frame(CUBS_TANK_DF)
  ...
  ## Create column names
colnames(ASTROS_TANK_DF) = c("Year", "Win%", "# Top 5 picks", "# SS Drafted", "# P Drafted", "# College Draftees", "# Draft Picks", "SS %",
                             "P %", "College %", "# Int'l Signings", "Int'l Signing Money Spent", "# FA Contracts", "Mean FA AAV", "Mean FA Length",
                             "Mean FA Total Value", "Team bWAR", "Team payroll")

colnames(CUBS_TANK_DF) = c("Year", "Win%", "# Top 5 picks", "# SS Drafted", "# P Drafted", "# College Draftees", "# Draft Picks", "SS %",
                           "P %", "College %", "# Int'l Signings", "Int'l Signing Money Spent", "# FA Contracts", "Mean FA AAV", "Mean FA Length",
                           "Mean FA Total Value", "Team bWAR", "Team payroll")
...
ASTROS_TANK_DF[1,3] = length( which( ASTROS_CUBS_DRAFT_SUB$Tm == "Houston Astros" & ASTROS_CUBS_DRAFT_SUB$Year == 2011 & ASTROS_CUBS_DRAFT_SUB$Rd <= 5 ) )
...
## Fill # of SSs drafted
ASTROS_TANK_DF[1,4] = length( which( ASTROS_CUBS_DRAFT_SUB$Tm == "Houston Astros" & ASTROS_CUBS_DRAFT_SUB$Year == 2011 & ASTROS_CUBS_DRAFT_SUB$POS == "SS" ) )
...
ASTROS_CUBS_DRAFT_SUB_min2011 = ASTROS_CUBS_DRAFT_SUB[ -which(ASTROS_CUBS_DRAFT_SUB$Year == "2011"), ]

ASTROS_CUBS_DRAFT_SUB_min2011[ which(ASTROS_CUBS_DRAFT_SUB_min2011$Year < 2016), ] %>%
  group_by(Tm) %>%
  summarize(sum(`Career WAR`, na.rm = T))
...
Rebuild_T
#write.csv(REBUILDERS_df, "C://Users//Erik//Desktop//BoxCopy//Programming Scripts and Data//Baseball//Projects//Tanking//REBUILDERSdf.csv")

Printable_REBUILD$WARPM = round(as.numeric(Printable_REBUILD$WAR) / as.numeric(Printable_REBUILD$Salary), digits = 3)
Printable_REBUILD$WiPM = round(as.numeric(Printable_REBUILD$W) / as.numeric(Printable_REBUILD$Salary), digits = 3)




#Draft_Results = PLAYER_CAREER_WAR %>%
#  left_join(BBA_DRAFT_ALL) %>%
#  group_by(Year, `Drafted By`) %>%
#  summarize(Draft_Class_Career_WAR = sum(`Career WAR`, na.rm = T))

#Draft_Results = as.data.frame(Draft_Results)

#Draft_Results[ ,3] = round(Draft_Results[ , 3], digits = 1)

#Draft_Results2 = PLAYER_CAREER_WAR %>%
#  left_join(BBA_DRAFT_ALL) %>%
#  group_by(`Drafted By`) %>%
#  summarize(Draft_Class_Career_WAR = sum(`Career WAR`, na.rm = T)) %>%
#  arrange(desc(Draft_Class_Career_WAR)) %>%
#  filter(!is.na(`Drafted By`))

#write.csv(Draft_Results, "C://Users//Erik//Desktop//BoxCopy//Programming Scripts and Data//Baseball//Projects//Tanking//CareerWARbyTeamDraftClass.csv")
#write.csv(Draft_Results2, "C://Users//Erik//Desktop//BoxCopy//Programming Scripts and Data//Baseball//Projects//Tanking//CareerWARbyTeam.csv")

##### Competitive Balance/Imbalance #####
`meanTmW%` = aggregate(x = StandALL$`W-L%`[ -which(StandALL$Year == 2020) ], by = list(StandALL$Tm[ -which(StandALL$Year == 2020) ]), FUN = mean)
`sdTmW%` = aggregate(x = StandALL$`W-L%`[ -which(StandALL$Year == 2020) ], by = list(StandALL$Tm[ -which(StandALL$Year == 2020) ]), FUN = sd)
WP_DF = matrix(ncol = 3, nrow = 30)
WP_DF = as.data.frame(WP_DF)
colnames(WP_DF)[1] = "Tm"
colnames(WP_DF)[2] = "MeanW%"
colnames(WP_DF)[3] = "sdW%"
WP_DF[,1] = as.character(unlist(`meanTmW%`))[1:30]
WP_DF[,2] = as.numeric(as.character(unlist(`meanTmW%`))[31:60])
WP_DF[,3] = as.numeric(as.character(unlist(`sdTmW%`))[31:60])


`meanLW%` = aggregate(x = StandALL$`W-L%`, by = list(StandALL$Year), FUN = mean)
`sdLW%` = aggregate(x = StandALL$`W-L%`, by = list(StandALL$Year), FUN = sd)
L_WP_DF = matrix(ncol = 3, nrow = 11)
L_WP_DF = as.data.frame(L_WP_DF)
colnames(L_WP_DF)[1] = "Year"
colnames(L_WP_DF)[2] = "LgMeanW%"
colnames(L_WP_DF)[3] = "LgsdW%"
L_WP_DF[,1] = as.character(unlist(`meanLW%`))[1:11]
L_WP_DF[,2] = as.numeric(as.character(unlist(`meanLW%`))[12:22])
L_WP_DF[,3] = as.numeric(as.character(unlist(`sdLW%`))[12:22])

  ## Get color code from takinggraphs script
W_P = ggplot(StandALL[ -which(StandALL$Year == 2020) ], aes(x = Tm, y = `W-L%`, fill = Tm)) +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        ...
        axis.text.y = element_text(color = c(color_code$currentcolor))) +
  geom_boxplot() + 
  scale_color_manual(values = c(color_code$currentcolor)) +
  scale_fill_manual(values = c(color_code$currentcolor)) +
  labs(x = "Team", y = "W %", title = "Distribution of Team W %s in MLB\n2011-2021") +
  coord_flip()
W_P


MOBILITY_W_P = matrix(nrow = 30, ncol = 9)
MOBILITY_W_P = as.data.frame(MOBILITY_W_P)
colnames(MOBILITY_W_P) = as.character(c("Tm", paste("r_",seq(2011,2018,1),"_",seq(2012,2019,1),sep = "")))
MOBILITY_W_P[,1] = color_code$currentTeam


listy = vector()
for (i in StandALL$Tm[order(unique(StandALL$Tm))]) {
  listy[i] = list(which(StandALL$Tm == i))
}

YOYWL = dcast(data = TeamValALL[-which(TeamValALL$Year == 2020),], formula = Tm~Year, value.var = "W_L")
YOYWAR = dcast(data = TeamValALL[-which(TeamValALL$Year == 2020),], formula = Tm~Year, value.var = "WAR")
YOYSal = dcast(data = TeamValALL[-which(TeamValALL$Year == 2020),], formula = Tm~Year, value.var = "Salary")
YOYTrades = dcast(data = TeamValALL[-which(TeamValALL$Year == 2020),], formula = Tm~Year, value.var = "Sum_Num_Trades")
YOYFAs = dcast(data = TeamValALL[-which(TeamValALL$Year == 2020),], formula = Tm~Year, value.var = "Sum_Num_FAs")

cor(t(wp_cor_mat[,2:12]))
cor(L_WP_DF[,3])

cleaned_br_hitter_val_df_list[[1]]